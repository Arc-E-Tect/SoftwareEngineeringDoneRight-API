...
defaults:
  openapi:
    lint: .redocly.yaml   # keep it outside sources.root
...
Any other YAML file under the source root counts as well, so keep the Redocly configuration beside `apionly.yaml`, outside `sources.root`.
