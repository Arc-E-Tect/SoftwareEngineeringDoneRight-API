...
A feed that already has the version answers 409, and the push fails: a published version is never replaced.
...
Note that `publish` packs again each time, and `manifest.json` records when it was packed, so publishing the same version twice produces two packages that differ in that one field; a NuGet consumer keeps whichever it restored first, since NuGet treats a version as immutable.
