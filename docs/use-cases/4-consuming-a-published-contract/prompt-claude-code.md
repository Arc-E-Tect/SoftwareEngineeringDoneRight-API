---
description: Make this Gradle project consume a published API contract with the API-Only Subscriber, test first.
---

# Consume a published API contract in this project, test first

Help me make this Gradle project build against an API contract that another team or company publishes, with the API-Only Subscriber.
The contract is published as an archive, built by the API-Only Publisher, to a Maven repository.
This project does not own the contract and does not change it.

When we are done, `./gradlew check`:

- fetches the contract at the version this project declares, verifies every document against the archive's manifest, and records the version and the hashes in a committed `apionly.lock`;
...
- **With two or more subscriptions in one project**, `processResources` fails with `Entry manifest.json is a duplicate`.
  The fix, after the `apiOnlySubscriber` block, puts each contract under `contracts/<target>/` on the classpath:

  ```groovy
  sourceSets.main.resources.setSrcDirs(['src/main/resources'])   // plus any other resource directories
...
2. The contracts to subscribe to: the repository URL, the group id, each target name and each version.
   Ask me for anything you cannot find.
3. How this project uses API descriptions today: committed copies, a generator, classpath access in code, and at which paths.
...
1. Add `apiContractVersion=<version>` to `gradle.properties`, with a comment saying it is the version of the contract this project builds against.
   With several contracts, give each subscription its own `version` instead, and tell me.
...
3. Create the test in the project's test package:
   ...
   With two or more contracts, read `/contracts/<target>/openapi.yaml` instead, one test per contract.
...
3. Add the `apiOnlySubscriber` block with the `maven` channel and the subscriptions.
4. With two or more subscriptions, add the classpath fix from the facts.
5. Point everything that read a description at the fetched document: the classpath resource, or `apiOnlySubscriber.subscription('<target>').openapi` for a Gradle task, never a path string.
   Remove any committed copy.
6. Tell me which Gradle properties to set locally and which environment variables CI needs, by name only.

## Step 4: Green

1. Run `./gradlew check`.
   Show `Subscribed to <target> <version>` for each contract, that `ApiContractTest` passes, and that the build succeeds.
2. Show `apionly.lock`.
...
3. Offer to add a section like this to `CLAUDE.md`, and write it only if I agree:
   ...
   - The API contracts this project builds against are fetched by the API-Only Subscriber into `build/api-spec/`. Never edit them, and never copy them into `src/`.
