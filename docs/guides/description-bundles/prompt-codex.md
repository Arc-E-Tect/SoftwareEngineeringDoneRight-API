...
- **Network access**: Gradle resolving the TranscriberJ, its emitter and the test libraries needs it, and so does the Publisher if this project builds its own contract.
...
Read only; do not edit anything in this step, except running the generation task.
Find out and report:
