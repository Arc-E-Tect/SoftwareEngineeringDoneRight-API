...
- Begin with a read-only investigation.
  Then present a plan that lists every file you will create or change in the current phase, and do not edit anything until I approve it.
- Keep your plan updated as you finish each phase and step.
...
- **Network access**: two things need it, `npm install` and Gradle resolving the Subscriber plugin.
...
3. Offer to add a section like this to `AGENTS.md`, and write it only if I agree:
   ...
   - Running `npm install` or a Gradle build that resolves plugins needs network access.
   ```
